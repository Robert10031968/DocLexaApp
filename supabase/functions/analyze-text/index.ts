...
serve(async (req) => {
  ...
  try {
    ...
    if (typeof wordsRaw !== 'number' || !Number.isFinite(wordsRaw)) {
      return badRequest('`words` must be a number')
    }
    const words = Math.max(0, Math.floor(wordsRaw))
    if (words <= 0) {
      return badRequest('`words` must be a positive integer')
    }

    const WORDS_PER_PAGE = readEnvInt('WORDS_PER_PAGE', 300)
    const pages = Math.ceil(Math.max(1, words / WORDS_PER_PAGE))
    ...
    console.error('analyze-text unexpected error:', err)
